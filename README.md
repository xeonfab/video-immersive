...
| `/sound-design` | Génère les prompts ElevenLabs (ambiance de fond par plan) et la structure du thème musical 24s. Ne décide plus du foley — voir `realisateur-ia` | Après la shot-list, avant génération vidéo ou en parallèle |
| `/realisateur-ia` | Réalisateur + directeur technique IA : exécute les prompts caméra via Artlist, puis analyse chaque rush généré pour décider s'il mérite un foley (et en écrit le prompt exact à partir du geste réellement animé, pas de la photo) avant de générer ambiance/foley/musique via ElevenLabs — devis et confirmation de budget avant toute dépense | Une fois les prompts (shot-list + ambiance) écrits, avant montage |
